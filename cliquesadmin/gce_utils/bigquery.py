...
# Not sure how to handle TIMESTAMP yet, might want to handle as int if
# it aways renders as a Unix timestamp
BQ_NP_TYPE_MAPPING = {
...
def query_response_to_dataframe(query_response):
    ...
    :param query_response: query response from
    :return:
    """
    rows = [tuple(map(lambda field: field['v'], row['f']))
            for row in query_response['rows']]

    dtypes = [(str(field['name']), BQ_NP_TYPE_MAPPING[field['type']])
